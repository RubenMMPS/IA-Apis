...
- **Frontend con identidad visual propia**: dashboard con estética inspirada en paneles de monitorización de sistemas (fondo oscuro, tipografía IBM Plex Sans/Mono, color con significado funcional por estado — no decorativo), responsive.
- **Validación de restricciones de la tarea**: Planner detecta restricciones técnicas explícitas en la petición del usuario (ej. "persistencia en memoria", "no usar SQLAlchemy") y las añade al plan; Architect las incorpora al diseño; Reviewer verifica su cumplimiento en el código final mediante dos mecanismos combinados: (1) juicio del LLM, y (2) un chequeo determinista de imports prohibidos (`app/agents/constraint_checks.py`) para las restricciones tecnológicas más comunes. Cualquiera de los dos que detecte una violación fuerza `changes_requested` — incluso si el LLM, por error, hubiera aprobado. **El chequeo determinista cubre solo un conjunto acotado de palabras clave/imports conocidos (SQLAlchemy y otros ORMs/drivers de DB comunes); para el resto de restricciones, la validación sigue dependiendo exclusivamente del juicio del LLM, sin garantía determinista.**
...
| `GET /tasks/{task_id}/events-history` | Historial completo de eventos persistidos de una tarea (para reconstruir el pipeline tras finalizar) |
...
- El sandbox soporta un conjunto de dependencias comunes para construcción de APIs REST: FastAPI, httpx, pytest-asyncio, SQLite + SQLAlchemy (vía `aiosqlite`), autenticación (`passlib[bcrypt]`, `pyjwt`), formularios (`python-multipart`) y configuración (`pydantic-settings`). No soporta motores de base de datos que requieran un servidor aparte (Postgres, MySQL) ni brokers/colas (Redis, Celery) porque el sandbox corre con `--network none`.
- **Nota de compatibilidad**: `bcrypt` se fija a `<4.1` en el sandbox porque versiones más recientes eliminaron un atributo interno (`__about__`) que `passlib` (sin mantenimiento activo) todavía espera — sin esa fijación, el hashing de contraseñas falla con un error de mensaje engañoso ("password cannot be longer than 72 bytes").
